...
class Products extends MY_Controller
{
	...
	function bom(){
        ...
        $this->page_construct('products/list_bom', $meta, $this->data);
	}
	...
	function add_bom(){
        ...
        if ($this->form_validation->run() == true) {
			...
			$date 				= $this->sma->fld(trim($_POST['date']));
            $data               = array(
									'name' 			=> $_POST['name'],
									'date' 			=> $date,
            ...
            for ($r = 0; $r < $j; $r++) {
				$to_item[] = array(
								...
								'unit_id'		=> $iterm_to_uom[$r],
								'status' 		=> 'add'
							);
				
            }
			
			$idConvert =  $this->products_model->insertBom($data,$from_item,$to_item);
			$this->session->set_flashdata('message', lang("bom_added"));
			redirect('products/bom');
        }else{
			...
			$this->page_construct('products/add_bom', $meta, $this->data);
		}
	...
	public function edit_bom($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			$idConvert = $this->products_model->updateBom($id, $data, $from_items, $to_items);
		...
		$this->data['all_bom'] = $this->products_model->getBom($id);
		$this->data['bom_froms'] = $this->products_model->getBomFrom($id);
		$this->data['bom_tos'] = $this->products_model->getBomTo($id);
		$this->data['id'] = $id;
        $this->page_construct('products/edit_bom', $meta, $this->data);
	}
	
	public function bom_actions()
    {
        if (!$this->Owner) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->products_model->deleteBom($id);
                    }
                    $this->session->set_flashdata('message', $this->lang->line("bom_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $bom = $this->products_model->getBomGroupId($id);
                        $user = $this->site->getUser($bom->created_by);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($bom->date));
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $bom->name);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $this->sma->formatDecimal($bom->qty));
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $user->first_name . ' ' . $user->last_name);
                        $row++;
                    }
                ...
                $this->session->set_flashdata('error', $this->lang->line("no_bom_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
    ...
    {	
        ...
        if ($this->products_model->deleteBom($id)) {
	...
	public function getListBom()
    {
        ...
        $detail_link = anchor('products/bom_note/$1', '<i class="fa fa-file-text-o"></i> ' . lang('product_analysis'), 'data-toggle="modal" data-target="#myModal2"');
        $edit_link = anchor('products/edit_bom/$1', '<i class="fa fa-edit"></i> ' . lang('edit_bom'));
        $delete_link = "<a href='#' class='po' title='<b>" . $this->lang->line("delete_bo") . "</b>' data-content=\"<p>"
        . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('products/delete_bom/$1') . "'>"
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			for ($r = 0; $r < $i; $r++) {
				 $product_unit_qty = $this->sma->formatDecimal($_POST['product_unit_qty'][$r]);
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $product_units)) {
    ...
    function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				$pr[] = array('id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'code' => $row->code, 'name' => $row->name, 'price' => $row->price, 'qty' => 1, 'uom' => $uom, 'cost' => $row->cost, 'options' => $options );
			}
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $product_units)) {
    ...
    {
        $this->sma->checkPermissions('items_convert', NULL, 'products');
        ...
        if ($this->form_validation->run() == true)
        {
            if ($this->Owner || $this->Admin || $this->Settings->allow_change_date == 1) {
                $date = $this->sma->fld($_POST['sldate']);
            } else {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if(!empty($cIterm_from_uom[$r])){
                    $product_unit= $this->site->getProductUnitByID($cIterm_from_uom[$r]);
                }
				
				$unit_qty 	  = ( !empty($product_unit->unit_qty) && $product_unit->unit_qty > 0 ? $product_unit->unit_qty : 1 );
				if($product_unit){
					$cost_unit    = $product_fr->cost * $unit_qty;
					$total_raw_cost += $cost_unit * $cIterm_from_qty[$r];
				} else {
					$cost_unit    = $product_fr->cost;
					$total_raw_cost += $cost_unit * $cIterm_from_qty[$r];
				}
				...
				$this->db->insert('sma_convert_items', $conItem);
			...
			//========================= Get Finish Qty ======================//
			for ($r = 0; $r < $j; $r++) {
				$option 	= $this->site->getProductUnitByID($iterm_to_uom[$r]);
				if($option){
					$total_fin_qty  += $iterm_to_qty[$r] * $option->unit_qty;
				}else{
            ...
            for ($r = 0; $r < $j; $r++) {
                ...
                if(!empty($iterm_to_uom[$r])){
                    $product_unit   = $this->site->getProductUnitByID($iterm_to_uom[$r]);
                }
				
                $unit_qty = ( !empty($iterm_to_uom[$r]) ? $product_unit->unit_qty : 1 );
				
				//============================ End ==============================//
				
				//========================== AVG Cost ===========================//
				if(!empty($iterm_to_uom[$r])){
					$qty_items 	= $iterm_to_qty[$r] * $product_unit->unit_qty;
				}else{
	...
	public function getDatabyBom_id()
	{
        ...
        if ($result) {
            ...
            foreach ($result as $row) {
				$options = $this->products_model->getUnitbyProduct($row->product_id);
                $pr[] = array('row' => $row, 'unit' => $options );
            }
            echo json_encode($pr);
        };
	}
	...
	function add_using_stock($count_id = NULL)
    {
        $this->sma->checkPermissions('adjustments', true);
        $this->form_validation->set_rules('warehouse', lang("warehouse"), 'required');
        ...
        if ($this->form_validation->run() == true && $this->products_model->addUsingStock($data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            ...
            $this->data['using_items'] = $count_id ? json_encode($pr) : FALSE;
            $this->data['warehouse_id'] = $count_id ? $stock_count->warehouse_id : FALSE;
            $this->data['count_id'] = $count_id;
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            ...
            $this->page_construct('products/add_using_stock', $meta, $this->data);

        }
    }
	
	function using_stocks($warehouse_id = NULL)
    {      
        if ($this->Owner || $this->Admin || !$this->session->userdata('warehouse_id')) {
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $this->data['warehouse'] = $warehouse_id ? $this->site->getWarehouseByID($warehouse_id) : null;
        ...
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('using_stocks')));
        $meta = array('page_title' => lang('using_stocks'), 'bc' => $bc);
        $this->page_construct('products/using_stocks', $meta, $this->data);
    }

	function getUsingStocks($warehouse_id = NULL)
    {        
        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_using_stock") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . site_url('products/delete_using_stock/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a>";

        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('using_stocks')}.id as id, date, reference_no, warehouses.name as wh_name, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as created_by, note, attachment")
            ->from('using_stocks')
        ...
        $this->datatables->add_column("Actions", "<div class='text-center'><a href='" . site_url('products/edit_using_stock/$1') . "' class='tip' title='" . lang("edit_using_stock") . "'><i class='fa fa-edit'></i></a> " . $delete_link . "</div>", "id");
	...
	function edit_using_stock($id)
    {
        $using_item = $this->products_model->getUsingStockByID($id);
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateUsingStock($id, $data, $products)) {
            $this->session->set_userdata('remove_qals', 1);
            $this->session->set_flashdata('message', lang("using_stock_updated"));
            redirect('products/using_stocks');
        } else {

            $inv_items = $this->products_model->getUsingStockItems($id);
            krsort($inv_items);
            $c = rand(100000, 9999999);
            foreach ($inv_items as $item) {
                $product = $this->site->getProductByID($item->product_id);
            ...
            $this->data['using_item'] = $using_item;
            $this->data['using_items'] = json_encode($pr);
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['warehouses'] = $this->site->getAllWarehouses();
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('products'), 'page' => lang('products')), array('link' => '#', 'page' => lang('edit_using_stock')));
	...
	public function view_using_stock($id)
    {
        ...
        $this->data['updated_by'] = $this->site->getUser($using_stock->updated_by);
