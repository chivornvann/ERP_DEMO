...
class Products_model extends CI_Model
{
	...
	public function getUnitbyProduct($pid,$baseunit){
		if($baseunit==''){
		...
		$q = $this->db->query("SELECT
								sma_units.id,
								sma_units.`name`,
								sma_product_units.unit_qty
							FROM
								`sma_units`
							LEFT JOIN sma_product_units ON sma_product_units.unit_id = sma_units.id
							AND sma_product_units.product_id = '".$pid."'
							WHERE
								base_unit = '".$baseunit."'
							OR sma_units.id = '".$baseunit."'	
					");
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
    ...
    public function addProduct($data, $items, $warehouse_qty, $product_attributes, $photos, $product_units)
    {
    ...
    {
        $this->db->select('' . $this->db->dbprefix('products') . '.id, code, unit,' . $this->db->dbprefix('products') . '.name as name, ' . $this->db->dbprefix('products') . '.price as price, ' . $this->db->dbprefix('product_variants') . '.name as vname')
	...
	public function getBomGroupId($id)
    {
        $this->db->select('date, name, sum(sma_bom_items.quantity) as qty, created_by');
		$this->db->from('bom');
    ...
    {
        $this->db->select('date, bom.name, bom_items.quantity, created_by, status, product_name, product_code, units.name as unit_name, products.quantity as qoh');
		$this->db->from('bom');
		$this->db->join('bom_items', 'bom_items.bom_id = bom.id');
		$this->db->join('products', 'products.id = bom_items.product_id', 'left');
		$this->db->join('units', 'bom_items.unit_id = units.id', 'left');
	...
	public function updateBom($id, $data, $from_items = array(),$to_items = array())
    {
		//$this->sma->print_arrays($data);
        $this->db->where('id', $id);
        if ($this->db->update('bom', $data)) {
            $this->db->delete('bom_items', array('bom_id' => $id));
			$this->db->insert_batch('bom_items', $from_items); 
			$this->db->insert_batch('bom_items', $to_items); 
        }
        return FALSE;
    }
	...
	public function insertBom($data,$from_items = array(),$to_items = array())
    {
		
        if ($this->db->insert('bom', $data)) {
            $convert_id = $this->db->insert_id();
            foreach ($from_items as $from_item) {
				 $from_item['bom_id'] = $convert_id;
				 $this->db->insert('bom_items', $from_item);
			}
			foreach ($to_items as $to_item) {
				 $to_item['bom_id'] = $convert_id;
				 $this->db->insert('bom_items', $to_item);
			}
			return true;
        }else{
			return false;
    ...
    public function updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants, $product_units)
    {
	...
	{
		$this->db->select('bom.*, bom_items.*, (SELECT COALESCE(quantity , 0) as qoh FROM sma_warehouses_products WHERE warehouse_id = '.$ware_id.' AND sma_warehouses_products.product_id = sma_bom_items.product_id) as qoh')
	...
	public function getUsingStockItems($using_stock_id)
    {
        $this->db->select('using_stock_items.*, products.code as product_code, products.name as product_name, products.image, products.details as details, product_variants.name as variant')
        ...
        $this->db->where('using_stock_id', $using_stock_id);

        $q = $this->db->get('using_stock_items');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
	...
	public function addUsingStock($data, $products)
    {
        if ($this->db->insert('using_stocks', $data)) {
            $using_id = $this->db->insert_id();
            foreach ($products as $product) {
                $product['using_stock_id'] = $using_id;
                $this->db->insert('using_stock_items', $product);
                $this->syncUsingStock($product);
            }
            if ($this->site->getReference('qa') == $data['reference_no']) {
                $this->site->updateReference('qa');
            }
    ...
    public function updateUsingStock($id, $data, $products)
    {
        $this->reverseUsingStock($id);
        if ($this->db->update('using_stocks', $data, array('id' => $id)) && 
            $this->db->delete('using_stock_items', array('using_stock_id' => $id))) {
            foreach ($products as $product) {
                $product['using_stock_id'] = $id;
                $this->db->insert('using_stock_items', $product);
                $this->syncUsingStock($product);
            }
            return true;
    ...
    public function deleteUsingStock($id)
    {
        $this->reverseAdjustment($id);
	...
	public function reverseUsingStock($id)
    {
        if ($products = $this->getUsingStockItems($id)) {
            foreach ($products as $using) {
                $where_clause = array('product_id' => $using->product_id, 'warehouse_id' => $using->warehouse_id, 'option_id' => $using->option_id, 'status' => 'received');
                if ($purchase_item = $this->site->getPurchasedItem($where_clause)) {
                    $quantity_balance = $using->type == 'subtraction' ? $purchase_item->quantity_balance + $using->quantity : $purchase_item->quantity_balance - $using->quantity;
                    $this->db->update('purchase_items', array('quantity_balance' => $quantity_balance), array('id' => $purchase_item->id));
                }

                $this->site->syncProductQty($using->product_id, $using->warehouse_id);
                if ($using->option_id) {
                    $this->site->syncVariantQty($using->option_id, $using->warehouse_id, $using->product_id);
                }
            }
        }
    }
	
	public function syncUsingStock($data = array())
    {
        if(! empty($data)) {
            $where_clause = array('product_id' => $data['product_id'], 'option_id' => $data['option_id'], 'warehouse_id' => $data['warehouse_id'], 'status' => 'received');
            if ($purchase_item = $this->site->getPurchasedItem($where_clause)) {
