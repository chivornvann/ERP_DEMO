<script type="text/javascript">
    ...
    $(document).ready(function () {
                ...
                if (ui.item.id !== 0) {
					...
					if(ui.item.uom !== false) {
						$.each(ui.item.uom, function () {
							$("<option />", {value: this.id, qty: this.unit_qty, text: this.name}).appendTo(opt);
						});
					} else {
        ...
        function checkItem(data){
            ...
            for(var i=0; i < data.length; i++){
                if(data[i].row.status == 'deduct'){
					...
					if(data[i].unit !== false) {
						$.each(data[i].unit, function () {
							if (data[i].row.unit_id == this.id)
								$("<option />", {value: this.id, text: this.name, qty: this.unit_qty, selected: 'selected'}).appendTo(opt);
							else
								$("<option />", {value: this.id, text: this.name, qty: this.unit_qty}).appendTo(opt);
						});
					} else {
						$("<option />", {value: 0, text: 'n/a'}).appendTo(opt);
						opt = opt.hide();
		...
		$(document).on('change', '#from_option',function(){
			var unit_qty = $("option:selected", this).attr("qty");
		});
