$(document).ready(function () {
	...
	$(document).on('change', '.unit_id', function () {
        var unit = $(this).val();
		var unit_qty = $('option:selected', this).attr('unit_qty');
		var parent_unit = $(this).parent().parent();
		parent_unit.find(".rquantity").val(getNumber(unit_qty));
    });
...
function loadItems() {
    ...
    if (localStorage.getItem('qaitems')) {
        ...
        $.each(sortedItems, function () {
			...
			var opt1 = $("<select id=\"poption\" name=\"unit_id\[\]\" class=\"form-control select unit_id\" />");
            if(item.unit !== false) {
                $.each(item.unit, function () {
                    if (item.row.unit == this.id)
                        $("<option />", {value: this.id, text: this.name, selected: 'selected', unit_qty: this.unit_qty}).appendTo(opt);
                    else
                        $("<option />", {value: this.id, text: this.name , unit_qty: this.unit_qty}).appendTo(opt1);
                });
            } else {
                $("<option />", {value: 0, text: 'n/a'}).appendTo(opt1);
                opt1 = opt1.hide();
            }					
            ...
            tr_html += '<td class="hidden"><select name="type[]" class="form-contol select rtype" style="width:100%;"><option value="subtraction"'+(type == 'subtraction' ? ' selected' : '')+'>'+type_opt.subtraction+'</option><option value="addition"'+(type == 'addition' ? ' selected' : '')+'>'+type_opt.addition+'</option></select></td>';
